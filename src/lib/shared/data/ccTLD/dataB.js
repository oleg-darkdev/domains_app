const data = [
  ...
  },
  {
    ...
    notes: [
      ...
      `Registrations are at the third level beneath several second-level labels, paralleling the oldest gTLDs; registration is open except in the gov and mil subdomains, which are limited to authorized entities in the Bangladesh government. Though online registration available, currently BTCL only allowing Second-level domain registration of .bd domain for only Bangladeshi citizens.`,
      `Means, It only allows the structure of websites like - example.com.bd, example2.com.bd. example3.com.bd; but not like - example.bd, example2.bd, example3.bd.`,
  ...
  },
  {
    ...
    notes: [
      ...
      `The .by code is used for domain hack by institutions from the German state of Bavaria (German: Bayern), such as bayern.by, the Bavarian Tourism Agency. Since "by" means "town" or "city" in Norwegian, it's also used by some Norwegian newspapers, such as osl.by for an Oslo newspaper, and trd.by for a Trondheim newspaper.`,
      `The BY code originates from the ISO code for Byelorussia, the former name of the country. The ccTLD was created for Belarus on 10 May 1994.`,
      `The Operations and Analysis Centre under the President of the Republic of Belarus allows for anyone (not only for those who reside in Belarus) to register a second level domain such as something.by. However, third-level domains .gov.by and .mil.by are reserved for government use only. `,
